...
We now want to compute $$\mathbb{E}[p_{\theta}(x,z)]$$ given the observed values of $$x$$ as well as a guess for the parameters that includes a guess of the mixture weights, that is we are interested in: $$\mathbb{E}[p_{\theta}(x,z) | \theta_0, x]$$. Treating $$\theta_0$$ and $$x$$ as given, the only random variable for this expectation is $$p_{\theta}(z | x, \theta_0)$$.
...
Given the observed data $$x$$ and our initial guess of $$\theta_0$$ and $$\hat{\pi}_j$$ we obtain a distribution that we can use to calculate $$\mathbb{E}[p_{\theta}(x,z) | \theta_0, x]$$
