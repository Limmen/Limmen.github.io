...
We now want to compute $$\mathbb{E}\[p_{\theta}(x,z)\]$$ given the observed values of $$x$$ as well as a guess for the parameters that includes a guess of the mixture weights, that is we are interested in:
