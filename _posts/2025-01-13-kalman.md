...
with state and observation spaces $$S = O= \mathbb{R}$$, discrete time $$t=0,1,\hdots$$, and coefficients $$a,c \in \mathbb{R}$$. We assume: (i) that $$v_t$$ and $$v_k$$ for any $$t$$ and $$k$$ are independent; (ii) $$w_t$$ and $$w_k$$ for any $$t$$ and $$k$$ are independent; (iii) $$(v_t)_{t \geq 0}$$ and $$(w_t)_{t=\geq 0}$$ are independent processes; and (iv) $$s_0$$ is independent of $$v_t$$ and $$w_t$$ for any $$t$$.
...
$$
\mathbf{b}_{t} = \mathcal{N}(\hat{s}_{t|t}, \Sigma_{t|t})\\
...
where $$\mathbf{b}_{t}$$ is the posterior at time $$t$$ and $$\hat{s}_{t|t}$$ is the state estimate at time $$t$$.
...
Hence, $$s_t$$ is a linear combination of the random variables $$s_0, w_0, w_1,\hdots,w_{t-1}$$. Since all of these variables are individually Gaussian and independent, they are jointly Gaussian. As Gaussians are closed under linear transformations, it follows that $$s_t$$ is Gaussian.
...
$$
p(s_1 \mid o_0) = \mathcal{N}(\hat{s}_{1|0}, \Sigma_{1|0})\\
p(o_1 \mid o_0) = \mathcal{N}(c\hat{s}_{1|0}, c^2\Sigma_{1|0} + \sigma_v^2)
$$
...
$$
p(s_1 \mid o_0, o_1) = \mathcal{N}(\hat{s}_{1|1}, \Sigma_{1|1})
$$
...
$$
p(s_k \mid \mathbf{o}_{0:k-1}) = \mathcal{N}(\hat{s}_{k|k-1}, \Sigma_{k|k-1})\\
p(s_k \mid \mathbf{o}_{0:k-1}) = \mathcal{N}(c\hat{s}_{k|k-1}, c^2\Sigma_{k|k-1} + \sigma_v^2)
$$

where $$\hat{s}_{k|k-1} = a\hat{s}_{k-1|k-1}$$ and $$\Sigma_{k|k-1} = a^2\Sigma_{k-1|k-1} + \sigma_w^2$$. Further
...
which means that $$p(x_k \mid \mathbf{o}_{0:k}) = \mathcal{N}(\hat{s}_{k|k}, \Sigma_{k|k})$$, where
