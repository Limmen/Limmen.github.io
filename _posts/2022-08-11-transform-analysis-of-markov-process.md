...
A finite discrete-time Markov process $$(X_t)_{t\geq 1}$$ is a stochastic process defined by $$\langle \mathcal{S}, \mathcal{P} \rangle$$ where $$\mathcal{S} = \{1, 2, ..., n\}$$ is the state space and $$\mathcal{P} \in \mathbb{R}^{n^2}$$
...
1-0.7z & 0.2z\\
0.3z & 1-0.8z
\end{bmatrix}=\frac{1}{1-1.5z + 0.5z^2}
\begin{bmatrix}
1-0.7z & 0.2z\\
0.3z & 1-0.8z
\end{bmatrix}
