...
A finite discrete-time Markov process $$(X_t)_{t\geq 1}$$ is a stochastic process defined by $$\langle \mathcal{S}, \mathcal{P} \rangle$$ where $$\mathcal{S} = \{1, 2, ...\}$$ is the state space and $$\mathcal{P} \in \mathbb{R}^{|\mathcal{S}|^2}$$ is a square row-stochastic transition matrix and the transitions $$X_t\rightarrow X_{t+1}$$ satisfies the Markov property:
